...
func HandleRetry(ch *amqp.Channel, d *amqp.Delivery) error {
	...
	log.Printf("retrying the message %s again, retry count is: %d", d.Body, retryCount)

	if retryCount >= MaxRetryCOunt {
		log.Printf("Moving the message to DLQ %s", DLQ)
	...
	return ch.PublishWithContext(context.Background(), d.Exchange, d.RoutingKey, false, false,
		amqp.Publishing{
			ContentType:  "application/json",
			Headers:      d.Headers,
			Body:         d.Body,
